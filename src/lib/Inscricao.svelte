<script>
    ...
    // Dados dos ficheiros para envio (Base64)
    let file1Data = null;
    let file2Data = null;
    ...
    async function handleFileChange1(e) {
        const file = e.target.files[0];
        if (!file) return;

        // Validação de 4MB (Limite da Vercel Free/Pro Functions)
        if (file.size > 4 * 1024 * 1024) {
            alert("O ficheiro é demasiado grande (Máx 4MB). Por favor usa um link externo (WeTransfer/Drive) ou comprime o PDF.");
            e.target.value = '';
            fileName1 = '';
            file1Data = null;
            return;
        ...
        file1Data = { name: file.name, content: base64 };
    }
    ...
    async function handleFileChange2(e) {
        ...
        if (file.size > 4 * 1024 * 1024) {
            alert("O ficheiro é demasiado grande (Máx 4MB).");
            e.target.value = '';
            fileName2 = '';
            file2Data = null;
            return;
        ...
        fileName2 = file.name;
        const base64 = await toBase64(file);
        file2Data = { name: file.name, content: base64 };
    }

    // --- ENVIO DO FORMULÁRIO ---
    async function handleSubmit(e) {
        submitting = true;
        statusMessage = 'A enviar inscrição...';
        statusType = 'info';
        ...
        const payload = {
            ...
            p2_ficheiro: file2Data,
            termos_aceites: formData.get('termos_aceites') === 'on'
        };

        try {
            // Envia para a API (api/enviar.js)
            ...
            console.error(error);
            statusMessage = 'Erro ao enviar. Verifica o tamanho dos ficheiros.';
            statusType = 'error';
        } finally {
            submitting = false;
        }
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            <label class="upload-area {fileName1 ? 'file-selected' : ''}">
                                <input type="file" name="p1_ficheiro" class="file-input" accept=".zip,.rar,.pdf" on:change={handleFileChange1} required />
                                {#if fileName1}
                                ...
                                {:else}
                                    ...
                                    <span class="upload-subtext">Formatos .ZIP ou .RAR (Max 4MB)</span>
                                {/if}
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            <label class="upload-area {fileName2 ? 'file-selected' : ''}">
                                <input type="file" name="p2_ficheiro" class="file-input" accept=".zip,.rar,.pdf" on:change={handleFileChange2} required />
                                {#if fileName2}
                                    <span class="upload-text" style="color: #000; font-weight: 700;">FICHEIRO:</span>
